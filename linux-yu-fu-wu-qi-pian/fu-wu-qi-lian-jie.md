...
### 免密登录
---
在观看这一部分内容前建议先跳转至[用户管理](#用户管理)，了解相关内容！
...
<figure><img src="../.gitbook/assets/linux_fuwuqi/image-20250709224003747 (1).png" alt=""><figcaption></figcaption></figure>
...
**每个用户必须属于一个“主组”**。通常在创建用户时，系统会自动创建一个**与用户名同名**的组作为其主组。
一个用户可以同时属于多个“附加组”。
将一个普通用户添加到这个特定的 `sudo` 组里，就赋予了该用户执行 `sudo` 命令的权力。
...
**4. 验证操作是否成功**
现在，我们需要确认用户 `username` 确实已经被添加到了正确的组中。
...
**5. 测试 `sudo` 权限：**
从 `root` 切换为普通用户 `username`
```bash
su - username
```
尝试执行一个特权命令，比如更新软件包列表。在命令前加上 `sudo` ;
